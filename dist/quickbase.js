...
class RecordsApi extends BaseAPI {
    ...
    /**
     * Upsert records (alternative endpoint)
     */
    async upsertRecordsRaw(requestParameters, initOverrides) {
        if (requestParameters['generated'] == null) {
            throw new RequiredError('generated', 'Required parameter "generated" was null or undefined when calling upsertRecords().');
        }
        const queryParameters = {};
        const headerParameters = {};
        headerParameters['Content-Type'] = 'application/json';
        const response = await this.request({
            ...
            headers: headerParameters,
            query: queryParameters,
            body: requestParameters['generated'].map(RecordToJSON),
        }, initOverrides);
        return new JSONApiResponse(response, (jsonValue) => jsonValue.map(Upsert200ResponseFromJSON));
    }
    /**
     * Upsert records (alternative endpoint)
     */
    async upsertRecords(requestParameters, initOverrides) {
        const response = await this.upsertRecordsRaw(requestParameters, initOverrides);
        return await response.value();
    }
}
