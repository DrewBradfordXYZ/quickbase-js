...
function quickbase(config) {
    ...
    const invokeMethod = async (methodName, params, retryCount = 0) => {
        const methodInfo = methodMap[methodName];
        if (!methodInfo) {
            throw new Error(`Method ${methodName} not found`);
        }
        const args = methodInfo.paramMap.length === 1 &&
            methodInfo.paramMap[0] === "requestParameters"
            ? [{ requestParameters: params }, undefined]
            : [params, undefined];
        let token = initialTempToken || (userToken && !useTempTokens ? userToken : undefined);
        ...
        if (useTempTokens && !token) {
            ...
            if (cachedToken) {
                ...
                if (typeof globalThis.window === "undefined" && !fetchApi) {
                    throw new Error("Temporary tokens require a browser environment or a custom fetchApi with browser-like session support");
                }
