// tests/vitest/qb/auth/ticketLocalStorageAuth.test.ts
...
test.skipIf(process.env.CI)(
  "QuickbaseClient Integration - runQuery with TicketLocalStorageSessionSource and 401 Retry Requiring Ticket Refresh (Mocked)",
  { timeout: 60000 },
  async () => {
    ...
    // Pre-populate localStorage with credentials
    localStorageMock.setItem(
      "quickbase-credentials",
      JSON.stringify({ username, password, appToken })
    );
    ...
    try {
      ...
      console.log(
        "TicketLocalStorageSessionSource: Successfully executed runQuery with 401 retry requiring ticket refresh."
      );
    } catch (error) {
