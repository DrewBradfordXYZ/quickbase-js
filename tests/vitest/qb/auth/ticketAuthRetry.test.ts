...
test.skipIf(process.env.CI)(
  ...
  async () => {
    ...
    // Mock getTempTokenDBID (retry with same ticket)
    mockFetch.mockImplementationOnce(async (url, options) => {
      console.log("[mockFetch] getTempTokenDBID (retry):", { url, options });
      if (
        url === `https://api.quickbase.com/v1/auth/temporary/${tableId}` &&
        options.headers["Authorization"] === "QB-TICKET integration-ticket-123"
      ) {
    ...
    try {
      ...
      // Verify ticket was not refreshed
      ...
      console.log(
        "TicketTokenStrategy: Successfully executed runQuery with 401 retry using valid ticket."
      );
    } catch (error) {
...
test.skipIf(process.env.CI)(
  ...
  async () => {
    ...
    // Mock getTempTokenDBID (401 error on token refresh)
    mockFetch.mockImplementationOnce(async (url, options) => {
      console.log("[mockFetch] getTempTokenDBID (retry):", { url, options });
      if (
        url === `https://api.quickbase.com/v1/auth/temporary/${tableId}` &&
    ...
    try {
      ...
      // Run query expecting 401 retry with ticket refresh
      const response = await client.runQuery({
        body: { from: tableId, select: [fieldId] },
      });
